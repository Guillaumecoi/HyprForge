...
def generate_rofi(binds):
    ...
    def format_group(headers, level=0, parent_meta_data=""):
        ...
        for header, subgroups in headers.items():
            ...
            if header:
                # Only print group headers as non-selectable lines
            ...
            if isinstance(subgroups, dict):
                ...
                for binding in subgroups:
                    # Skip printing a group header as a regular entry if it matches the header name
                    if binding.split(" ::: ")[0].strip() == header:
                        continue
